...
class MyForm : Form
{
    static void Main(String[] args)
    {
        ...
        Application.Run(new MyForm(title, name));
    }
    ...
    private void SetupComponents()
    {
        ...
        this.RecursevlyFillControlsName(this.lbControls, this.Controls);
    }
    ...
    /**
     * Recursevly fill a list box with the names of all the controls in the collection
     */
    private void RecursevlyFillControlsName(ListBox list, Control.ControlCollection controls, string prefix = "")
    {
        foreach(Control control in controls)
        {
            string controlName = prefix + (string.IsNullOrWhiteSpace(control.Name) ? control.GetType().Name : control.Name);

            list.Items.Add(controlName);

            if (control.Controls.Count > 0)
                this.RecursevlyFillControlsName(list, control.Controls, controlName + " -> ");
        }
    }
}